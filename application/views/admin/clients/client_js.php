...
$(document).ready(function(){
 ...
 e.preventDefault();
 var cnpj = $("#vat").val().replace(/[^0-9]/g, '');
      if(cnpj.length == 14) {
      $.ajax({
 ...
 // Agora preenchemos os campos com os valores retornados
		$('#company').val(response.nome);
		$('#fantasia').val(response.fantasia);
		$("#phonenumber").val(response.telefone);
        	$("#address").val(response.logradouro +' ' + response.numero +' ' +  response.complemento);
        	$("#bairro").val(response.bairro);
		$("#city").val(response.municipio);
        	$("#state").val(response.uf);
		$("#zip").val(response.cep);
