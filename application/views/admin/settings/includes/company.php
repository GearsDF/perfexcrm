<?php defined('BASEPATH') or exit('No direct script access allowed'); ?>
    ...
    <div role="tabpanel" class="tab-pane" id="company_info">
		...
		<?php echo render_input('settings[company_ie]','company_ie_number',get_option('company_ie')); ?>
        <?php echo render_input('settings[invoice_company_address]','settings_sales_address',get_option('invoice_company_address')); ?>
        <?php echo render_input('settings[invoice_company_bairro]','settings_sales_bairro',get_option('invoice_company_bairro')); ?>
        ...
        <hr />
        <?php echo 
render_textarea('settings[company_info_format]','company_info_format',clear_textarea_breaks(get_option('company_info_format')),array('rows'=>8,'style'=>'line-height:20px;')); 
?>
        <p>
			...
			<a href="#" class="settings-textarea-merge-field" data-to="company_info_format">{ie_number}</a>,
            <a href="#" class="settings-textarea-merge-field" data-to="company_info_format">{address}</a>,
            <a href="#" class="settings-textarea-merge-field" data-to="company_info_format">{company_bairro}</a>,
        ...
        if(count($custom_company_fields) > 0){
            ...
            foreach($custom_company_fields as $field){
                echo '<li class="list-group-item"><b>'.$field['name']. '</b>: ' . '<a href="#" class="settings-textarea-merge-field" 
data-to="company_info_format">{cf_'.$field['id'].'}</a></li>';
            }
            echo '</ul>';
            echo '<hr />';
        }
